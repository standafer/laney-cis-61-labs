...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        ...
        if rolled_one:
            # Already rolled one, no point in doing useless
            # computation
            continue

        if dice_result == 1:
            # Pig out rule: We need to call dice exactly num_rolls times so can't
            # just do an early return here
            rolled_one = True
        else:
            summation += dice_result
    
    if rolled_one == True:
        # Pig out
        return 1

    # Otherwise, just return the summation
    return summation

def get_highest_digit(num):
    # Desmos to demonstrate: https://www.desmos.com/calculator/zlmvjhhrxk
    # Get highest power of 10 required to recreate num

    # Ternerary operator to prevent OOB error for log10
    highest_exp_10 = floor(log10(num)) if num > 0 else 0
    highest_digit = -1

    # For each power, check the digit
    for i in range(0, highest_exp_10 + 1):
        digit = floor(
            (num / (10**i)) % 10
        )
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        # Free bacon rule: Scores the largest digit + 1
        highest_digit = get_highest_digit(opponent_score)
        return highest_digit + 1
    
    # Otherwise, it's just a normal play
...
def select_dice(score, opponent_score):
    ...
    True
    """
    summative_score = score + opponent_score
    if summative_score % 7 == 0:
        # Hog wild rule: summative score is a multiple of seven, must play
        # a four sided die
        return four_sided
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    # Switched to use an array because of the increased verbosness it provides
    # in code. Also eliminates a lot of conditional operators
    strategy_by_player = [strategy0, strategy1]

    while (scores[0] < goal and scores[other(0)] < goal):
        current_score = scores[who]
        opponent_score = scores[other(who)]

        selected_dice = select_dice(current_score, opponent_score)
        strategy = strategy_by_player[who]
        num_rolls = strategy(current_score, opponent_score)

        point_delta = take_turn(num_rolls, opponent_score, selected_dice)

        scores[who] += point_delta

        # Swapping logic
        if (scores[who] * 2 == scores[other(who)]
            or scores[other(who)] * 2 == scores[who]):
            # This order is not strict. It could even be [0], [1] = [1], [0] (direct index)
            # Though, I like the verbosity and consistency of using who and other(who)
            scores[who], scores[other(who)] = scores[other(who)], scores[who]

        who = other(who)

    return scores[0], scores[other(0)]
...
def max_scoring_num_rolls(dice=six_sided, verbose=True):
    ...
    max_average = -1
    max_dice = -1
    get_average = make_averaged(roll_dice)
    for i in range(1, 10 + 1):
        average = get_average(i, dice)
        if verbose:
            print(str(i) + " dice scores " + str(average) + " on average")
...
def bacon_strategy(score, opponent_score):
    ...
    potential_delta = get_highest_digit(opponent_score) + 1
    if potential_delta >= BACON_MARGIN:
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    potential_new_score = (score + potential_delta)
    swappable = (potential_new_score * 2 == opponent_score) or (opponent_score * 2 == potential_new_score)

    if swappable:
        if opponent_score > potential_new_score:
            return 0
    elif potential_delta >= BACON_MARGIN:
        return 0
...
def final_strategy(score, opponent_score):
    """
    Plays slightly risky during early-mid game, but in the end game switches
    to a much safer bacon relient strategy, tending to always take the bacon
    """
    
    potential_bacon = get_highest_digit(opponent_score) + 1
    potential_new_score = score + potential_bacon
    
    bacon_swaps = (potential_new_score * 2 == opponent_score) or (opponent_score * 2 == potential_new_score)
    hog_wild = (score + opponent_score) % 7 == 0
    
    points_to_win = 100 - score

    if bacon_swaps and opponent_score > potential_new_score:
        # Take swap if it's advantageous
        return 0
    
    if bacon_swaps and opponent_score < potential_new_score:
        # Avoid harmful swap
        return 5 if hog_wild else 6
    
    if points_to_win <= 20:
        # Close to winning, rely mostly on bacons
        if potential_bacon >= 4:
            # Lower bacon threshold
            return 0
        elif points_to_win <= potential_bacon:
            # Take bacon if it's enough to win
            return 0
        elif hog_wild:
            return 3

    # Otherwise, we're early-mid game, so play a little risky
    if hog_wild:
        return 4
