...
class ProfessorCard(Card):
	...
	def effect(self, other_card, player, opponent):
		...
		for player_card in player.deck.cards:
			player_card.defense += opponent.defense
			player_card.attack += opponent.attack
		
		# iterate backwards to preserve array order
		for i in range(len(opponent.deck.cards) - 1, 0 - 1, -1):
